...
// src/verify-enhanced.ts
var import_node_path13 = require("node:path");

// src/verify-core-legacy.ts
...
async function runCommand(command, args, options) {
  return await new Promise((resolve3, reject) => {
    ...
    child.on("close", (code) => {
      clearTimeout(timer);
      resolve3({ exitCode: code ?? 1, stdout, stderr, timedOut });
    });
  });
}
...
function sanitizedEnvironment(runId, temporaryHome = (0, import_node_os.tmpdir)()) {
...
async function diffPatch(cwd, base, head) {
  return await git(cwd, ["diff", "--no-ext-diff", "--unified=3", base, head, "--"]);
}
async function showFile(cwd, ref, path) {
...
async function changedFiles(cwd, base, head) {
  ...
  return output.split(/\r?\n/u).filter(Boolean).map((line) => {
    ...
    const status = code.startsWith("A") ? "added" : code.startsWith("M") ? "modified" : code.startsWith("D") ? "deleted" : code.startsWith("R") ? "renamed" : "unknown";
    const result = { path, status, category: classifyFile(path) };
    if (code.startsWith("R") && parts[1]) result.oldPath = parts[1];
    return result;
  });
}
...
// src/detectors-core.ts
var testDeclaration = /\b(?:test|it|describe)\s*\(|^\s*(?:async\s+)?def\s+test_/u;
var assertion = /\b(?:expect\s*\(|assert(?:\.|\s*\())|\bassert\s+/u;
function detectIntegrityFindings(patch, files) {
  const findings = [];
  const lines = parseDiffLines(patch);
  for (const file of files) {
    if (file.category === "test" && file.status === "deleted") findings.push(finding("N0-TEST-001", "test_file_deleted", "blocking", "A test file was deleted.", file.path));
    if (file.category === "ci") findings.push(finding("N0-CI-001", "ci_configuration_changed", "warning", "CI configuration changed and requires trusted-policy review.", file.path));
  }
  for (const line of lines) detectLine(line, findings);
...
function detectLine(line, findings) {
  ...
  if (added && category === "test" && (/\b(?:test|it|describe)\.(?:skip|todo)\s*\(/u.test(line.content) || /@pytest\.mark\.(?:skip|xfail)|@unittest\.skip/u.test(line.content))) findings.push(at(line, "N0-TEST-002", "test_skip_added", "blocking", "A skipped, todo, or expected-failure test was added."));
  if (added && category === "test" && /\b(?:test|it|describe)\.only\s*\(/u.test(line.content)) findings.push(at(line, "N0-TEST-003", "focused_test_added", "blocking", "A focused .only test was added and can exclude the rest of the suite."));
  if (!added && category === "test" && testDeclaration.test(line.content)) findings.push(at(line, "N0-TEST-004", "test_case_removed", "warning", "A test declaration was removed. Confirm that the removal is intentional."));
  if (!added && category === "test" && assertion.test(line.content)) findings.push(at(line, "N0-TEST-005", "assertion_removed", "warning", "An assertion was removed."));
  if (added && ["configuration", "test_configuration", "build_configuration", "dependency", "ci"].includes(category) && /(?:npm\s+(?:run\s+)?test|pnpm\s+(?:run\s+)?test|yarn\s+test|pytest|vitest|jest|node\s+--test)[^\n]*(?:\|\|\s*true|\|\|\s*exit\s+0|;\s*exit\s+0|\$LASTEXITCODE\s*=\s*0)/iu.test(line.content)) findings.push(at(line, "N0-CMD-001", "test_failure_masked", "blocking", "The test command masks a failing exit status."));
  if (added && /passWithNoTests\s*[:=]\s*true|--passWithNoTests/iu.test(line.content)) findings.push(at(line, "N0-TEST-006", "empty_suite_allowed", "warning", "The test configuration now allows an empty test suite to pass."));
  if (added && /catch\s*(?:\([^)]*\))?\s*\{\s*(?:\/\/[^\n]*)?$|except(?:\s+Exception)?\s*:\s*pass/iu.test(line.content)) findings.push(at(line, "N0-ERR-001", "error_swallowed", "warning", "A broad error handler appears to ignore an error."));
  if (added && category === "ci" && /pull_request_target\s*:/u.test(line.content)) findings.push(at(line, "N0-CI-002", "untrusted_privileged_trigger", "blocking", "pull_request_target can expose privileged context to untrusted pull-request code."));
  if (added && /(?:retry|retries)\s*[:=]\s*(?:[5-9]|\d{2,})|timeout\s*[:=]\s*(?:[6-9]\d{4}|\d{6,})/iu.test(line.content)) findings.push(at(line, "N0-TEST-009", "timeout_or_retry_increased", "warning", "A high timeout or retry value may conceal instability."));
}
function detectAssertionWeakening(lines, findings) {
  ...
  for (const weak of added) {
    ...
    if (precise) findings.push({ ...at(weak, "N0-TEST-007", "assertion_weakened", "blocking", "A precise assertion appears to have been replaced by a weaker truthiness or existence check."), evidence: `${precise.content.trim()} -> ${weak.content.trim()}` });
  }
}
function detectThresholdReduction(lines, findings) {
  ...
  for (const added of lines.filter((line) => line.kind === "added")) {
    ...
    if (Number.isFinite(oldNumber) && newNumber < oldNumber) findings.push(at(added, "N0-COV-001", "coverage_reduced", "blocking", `A coverage threshold appears to decrease from ${oldNumber} to ${newNumber}.`));
  }
}
function detectSnapshotVolume(lines, findings) {
  ...
  if (snapshotChanges >= 50 && snapshotChanges > productionChanges * 5) findings.push(finding("N0-SNAPSHOT-001", "snapshot_bulk_update", "warning", `${snapshotChanges} snapshot lines changed relative to ${productionChanges} production lines.`));
}
function finding(ruleId, kind, severity, message, path) {
  return { ruleId, kind, severity, message, ...path ? { path } : {} };
}
function at(line, ruleId, kind, severity, message) {
  return { ruleId, kind, severity, path: line.path, line: line.line, message, evidence: line.content.trim() };
}
function deduplicate(findings) {
  const seen = /* @__PURE__ */ new Set();
  return findings.filter((item) => {
    const key = `${item.ruleId}:${item.path ?? ""}:${item.line ?? ""}`;
...
// src/detectors.ts
function detectIntegrityFindings2(patch, files) {
  ...
  for (const added of lines.filter((line) => line.kind === "added")) {
    ...
    const previous = previousLine ? threshold(previousLine.content) : void 0;
  ...
  return deduplicate2(findings);
}
function threshold(content) {
  const match = /\b(threshold|branches|functions|lines|statements)\b[^\d]*(\d+(?:\.\d+)?)/iu.exec(content);
  return match?.[1] && match[2] ? { key: match[1].toLowerCase(), value: Number(match[2]) } : void 0;
}
function deduplicate2(findings) {
  const seen = /* @__PURE__ */ new Set();
  return findings.filter((finding2) => {
    const key = `${finding2.ruleId}:${finding2.path ?? ""}:${finding2.line ?? ""}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
...
async function runCounterfactual(options) {
  ...
  const changedTests = options.files.filter(
    (file) => file.category === "test" && (file.status === "added" || file.status === "modified")
  );
  ...
  const temporaryRoot = await (0, import_promises.mkdtemp)((0, import_node_path.join)((0, import_node_os2.tmpdir)(), "n0-verify-"));
  const baseDirectory = (0, import_node_path.join)(temporaryRoot, "base-with-head-tests");
  const headDirectory = (0, import_node_path.join)(temporaryRoot, "head");
  try {
    ...
    for (const test of changedTests) {
      ...
      const destination = (0, import_node_path.join)(baseDirectory, ...test.path.split("/"));
      await (0, import_promises.mkdir)((0, import_node_path.dirname)(destination), { recursive: true });
      await (0, import_promises.writeFile)(destination, content, "utf8");
    }
    ...
    if (headRun.exitCode !== 0 || headRun.timedOut) {
      return {
        ...
        summary: headRun.timedOut ? "The candidate test run timed out." : "The candidate test run failed before a useful counterfactual comparison.",
    ...
    await (0, import_promises.rm)(temporaryRoot, { recursive: true, force: true });
  }
}
async function addWorktree(repository, path, ref) {
  ...
  if (result.exitCode !== 0) throw new Error(result.stderr.trim() || "Could not create a Git worktree.");
}
async function removeWorktree(repository, path) {
  await runCommand("git", ["worktree", "remove", "--force", path], { cwd: repository }).catch(() => void 0);
}
...
async function loadTrustedConfig(repository, base) {
  for (const path of [".n0/n0.config.yml", ".n0/n0.config.yaml"]) {
    try {
      const text = await showFile(repository, base, path);
      return { config: normalizeConfig((0, import_yaml.parse)(text)), source: `${base}:${path}`, trusted: true };
    } catch {
    }
  }
  for (const path of [(0, import_node_path2.join)(repository, ".n0", "n0.config.yml"), (0, import_node_path2.join)(repository, ".n0", "n0.config.yaml")]) {
    try {
      const text = await (0, import_promises2.readFile)(path, "utf8");
      return { config: normalizeConfig((0, import_yaml.parse)(text)), source: path, trusted: false };
    } catch {
    }
  }
...
function normalizeConfig(value) {
  ...
  const input = value ?? {};
  if (input.version !== void 0 && input.version !== 1) throw new Error("Unsupported N0 configuration version.");
  ...
  return {
    ...
    policies: {
      ...
      minimumSeverity: typeof policies.minimum_severity === "string" ? policies.minimum_severity : defaultConfig.policies.minimumSeverity
    },
...
// src/config.ts
async function loadTrustedConfig2(repository, base) {
  const organizationPolicy = process.env.N0_ORG_POLICY;
  if (organizationPolicy) {
    const path = (0, import_node_path3.resolve)(organizationPolicy);
    return { config: normalizeConfig2((0, import_yaml2.parse)(await (0, import_promises3.readFile)(path, "utf8"))), source: `organization:${path}`, trusted: true };
  }
  const loaded = await loadTrustedConfig(repository, base);
  return { ...loaded, config: normalizeConfig2(loaded.config) };
}
...
function validateConfigShape(value) {
  ...
  if (value.version !== void 0 && value.version !== 1) throw new Error("Unsupported N0 configuration version.");
  const execution = isRecord(value.execution) ? value.execution : {};
  if (execution.mode !== void 0 && !["audit", "worktree", "container"].includes(String(execution.mode))) throw new Error("execution.mode must be audit, worktree, or container.");
  for (const key of ["timeout_seconds", "timeoutSeconds", "max_memory_mb", "maxMemoryMb", "max_cpu", "maxCpu"]) {
    if (execution[key] !== void 0 && (typeof execution[key] !== "number" || !Number.isFinite(execution[key]) || Number(execution[key]) < 0)) throw new Error(`execution.${key} must be a non-negative number.`);
  }
  const commands = isRecord(value.commands) ? value.commands : {};
  for (const [key, command] of Object.entries(commands)) if (command !== void 0 && typeof command !== "string") throw new Error(`commands.${key} must be a string.`);
  ...
  for (const key of ["block_on", "blockOn", "warn_on", "warnOn", "critical_paths", "criticalPaths"]) {
    if (policies[key] !== void 0 && (!Array.isArray(policies[key]) || !policies[key].every((item) => typeof item === "string"))) throw new Error(`policies.${key} must be an array of strings.`);
  }
}
...
var javascriptAdapter = {
  ...
  async detect(directory) {
    return await fileExists((0, import_node_path4.join)(directory, "package.json")) ? 100 : 0;
  },
  async discoverCommands(directory) {
    const packageJson = JSON.parse(await (0, import_promises4.readFile)((0, import_node_path4.join)(directory, "package.json"), "utf8"));
    const manager = await detectPackageManager(directory);
    const run = (name) => packageJson.scripts?.[name] ? `${manager} ${manager === "yarn" ? "" : "run "}${name}`.replace(/\s+/gu, " ").trim() : void 0;
    const install = manager === "npm" ? await fileExists((0, import_node_path4.join)(directory, "package-lock.json")) ? "npm ci --ignore-scripts" : "npm install --ignore-scripts" : manager === "pnpm" ? "pnpm install --frozen-lockfile --ignore-scripts" : "yarn install --frozen-lockfile --ignore-scripts";
...
var pythonAdapter = {
  id: "python",
  async detect(directory) {
    for (const name of ["pyproject.toml", "pytest.ini", "requirements.txt", "setup.py"]) if (await fileExists((0, import_node_path4.join)(directory, name))) return 90;
    return 0;
  },
  async discoverCommands(directory) {
    const manager = await fileExists((0, import_node_path4.join)(directory, "uv.lock")) ? "uv" : await fileExists((0, import_node_path4.join)(directory, "poetry.lock")) ? "poetry" : "pip";
    const install = manager === "uv" ? "uv sync --frozen" : manager === "poetry" ? "poetry install --no-interaction" : await fileExists((0, import_node_path4.join)(directory, "requirements.txt")) ? "python -m pip install -r requirements.txt" : void 0;
...
var goAdapter = {
  id: "go",
  async detect(directory) {
    return await fileExists((0, import_node_path4.join)(directory, "go.mod")) ? 80 : 0;
  },
  async discoverCommands() {
    return { adapter: "go", packageManager: "go", build: "go build ./...", test: "go test ./...", coverage: "go test -coverprofile=coverage.out ./..." };
  },
...
var rustAdapter = {
  id: "rust",
  async detect(directory) {
    return await fileExists((0, import_node_path4.join)(directory, "Cargo.toml")) ? 80 : 0;
  },
  async discoverCommands(directory) {
    const locked = await fileExists((0, import_node_path4.join)(directory, "Cargo.lock"));
    return { adapter: "rust", packageManager: "cargo", build: `cargo build${locked ? " --locked" : ""}`, test: `cargo test${locked ? " --locked" : ""}` };
  },
  isTestFile: (path) => /(^|\/)tests\/.*\.rs$/iu.test(path),
  isProductionFile: (path) => /\.rs$/iu.test(path) && !rustAdapter.isTestFile(path)
};
async function detectAdapter(directory, config, additional = []) {
...
function mergeCommandPlan(discovered, configured) {
  return { ...discovered, ...Object.fromEntries(Object.entries(configured).filter(([, value]) => value)) };
}
async function detectPackageManager(directory) {
  if (await fileExists((0, import_node_path4.join)(directory, "pnpm-lock.yaml"))) return "pnpm";
  if (await fileExists((0, import_node_path4.join)(directory, "yarn.lock"))) return "yarn";
  return "npm";
}
...
async function executePlan(options) {
  const entries = options.testOnly ? [["install", options.plan.install], ["test", options.plan.test]] : [["install", options.plan.install], ["build", options.plan.build], ["lint", options.plan.lint], ["typecheck", options.plan.typecheck], ["test", options.plan.test]];
  const commands = [];
  ...
  const status = commands.length === 0 ? "NOT_RUN" : commands.some((command) => command.status === "TIMEOUT") ? "TIMEOUT" : commands.every((command) => command.status === "PASS") ? "PASS" : "FAIL";
...
async function executeEvidenceCommand(options) {
  const started = Date.now();
  const startedAt = new Date(started).toISOString();
  const timeoutMs = options.config.execution.timeoutSeconds * 1e3;
  const runId = `${options.name}-${started}`;
  const result = options.mode === "container" ? await runContainerCommand(options.command, options.directory, options.config, timeoutMs, runId) : await runShellCommand(options.command, { cwd: options.directory, timeoutMs, env: sanitizedEnvironment(runId, options.directory) });
  const finished = Date.now();
  return {
    id: `RUN-${(0, import_node_crypto.createHash)("sha256").update(`${runId}:${options.command}`).digest("hex").slice(0, 12)}`,
    name: options.name,
...
async function runContainerCommand(command, directory, config, timeoutMs, runId) {
  ...
  return await runCommand("docker", [
    ...
    network,
    "--memory",
    `${config.execution.maxMemoryMb}m`,
    "--cpus",
    String(config.execution.maxCpu),
    "--pids-limit",
    "256",
    "--cap-drop",
    "ALL",
    ...
    "--tmpfs",
    `/workspace:rw,exec,nosuid,nodev,uid=65532,gid=65532,mode=0700,size=${workspaceSizeMb}m`,
    "--tmpfs",
    "/tmp:rw,nosuid,nodev,uid=65532,gid=65532,mode=1777,size=256m",
    "-e",
    "CI=true",
    "-e",
    `N0_RUN_ID=${runId}`,
...
// src/runner.ts
async function executePlan2(options) {
  try {
    return await executePlan(options);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    ...
    return {
      ...
      ref: options.ref,
      status: "BLOCKED",
      summary: `Execution environment blocked the run: ${message}`,
      commands: [{
        id: `RUN-${(0, import_node_crypto2.createHash)("sha256").update(message).digest("hex").slice(0, 12)}`,
        name: "environment",
        command: "",
        cwd: options.directory,
        status: "BLOCKED",
        ...
        stdout: "",
        stderr: message,
        stdoutHash: `sha256:${(0, import_node_crypto2.createHash)("sha256").update("").digest("hex")}`,
        stderrHash: `sha256:${(0, import_node_crypto2.createHash)("sha256").update(message).digest("hex")}`
      }]
    };
  }
}
...
// src/matrix.ts
async function runFourWayMatrix(options) {
  ...
  const root = await (0, import_promises5.mkdtemp)((0, import_node_path5.join)((0, import_node_os3.tmpdir)(), "n0-matrix-"));
  const directories = { C0T0: (0, import_node_path5.join)(root, "c0t0"), C1T0: (0, import_node_path5.join)(root, "c1t0"), C0T1: (0, import_node_path5.join)(root, "c0t1"), C1T1: (0, import_node_path5.join)(root, "c1t1") };
  const cells = {};
  try {
    await Promise.all([
      addWorktree2(options.repository, directories.C0T0, options.base),
      addWorktree2(options.repository, directories.C1T0, options.head),
      addWorktree2(options.repository, directories.C0T1, options.base),
      addWorktree2(options.repository, directories.C1T1, options.head)
    ]);
    await Promise.all([
    ...
    const keys = ["C0T0", "C1T0", "C0T1", "C1T1"];
    const executed = await Promise.all(keys.map(async (key) => [key, await executePlan2({ directory: directories[key], ref: key.startsWith("C0") ? options.base : options.head, snapshot: key.toLowerCase(), plan: options.plan, config: options.config, testOnly: true })]));
    ...
    await Promise.all(Object.values(directories).map((directory) => removeWorktree2(options.repository, directory)));
    await (0, import_promises5.rm)(root, { recursive: true, force: true });
  }
}
async function applyTestVersion(repository, directory, tests, ref, version) {
  for (const file of tests) {
    const shouldExist = version === "base" ? file.status !== "added" : file.status !== "deleted";
    const destination = (0, import_node_path5.join)(directory, ...file.path.split("/"));
    if (!shouldExist) {
      await (0, import_promises5.unlink)(destination).catch(() => void 0);
      continue;
    }
    ...
    await (0, import_promises5.writeFile)(destination, content, "utf8");
  }
}
function interpret(cells) {
  const status = (key) => cells[key]?.status;
...
async function addWorktree2(repository, path, ref) {
  const result = await runCommand("git", ["worktree", "add", "--detach", path, ref], { cwd: repository });
  if (result.exitCode !== 0) throw new Error(result.stderr.trim() || "Could not create matrix worktree.");
}
async function removeWorktree2(repository, path) {
  await runCommand("git", ["worktree", "remove", "--force", path], { cwd: repository }).catch(() => void 0);
}
...
var ignored = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "coverage", ".n0", "target"]);
async function inventoryTests(directory, adapter) {
  const files = [];
  for (const absolute of await walk(directory)) {
    const path = (0, import_node_path6.relative)(directory, absolute).replaceAll("\\", "/");
    if (!adapter.isTestFile(path)) continue;
    try {
      const content = await (0, import_promises6.readFile)(absolute, "utf8");
...
function inventoryJavascript(path, content) {
  return { path, framework: content.includes("vitest") ? "vitest" : content.includes("node:test") ? "node:test" : "jest/mocha", suites: matches(content, /\bdescribe(?:\.(?:skip|only))?\s*\(\s*["'`]([^"'`]+)/gu), tests: matches(content, /\b(?:test|it)(?:\.(?:skip|only|todo))?\s*\(\s*["'`]([^"'`]+)/gu), assertions: count(content, /\bexpect\s*\(|\bassert(?:\.|\s*\()/gu), skipped: count(content, /\b(?:test|it|describe)\.(?:skip|todo)\s*\(/gu), focused: count(content, /\b(?:test|it|describe)\.only\s*\(/gu) };
}
function inventoryPython(path, content) {
  return { path, framework: content.includes("pytest") ? "pytest" : "unittest", suites: matches(content, /^class\s+(Test\w+)/gmu), tests: matches(content, /^\s*def\s+(test_\w+)/gmu), assertions: count(content, /\bassert\s+|\.assert[A-Z]\w*\s*\(/gu), skipped: count(content, /@pytest\.mark\.(?:skip|xfail)|@unittest\.skip/gu), focused: 0 };
}
function inventoryGo(path, content) {
  return { path, framework: "go test", suites: [], tests: matches(content, /^func\s+(Test\w+)\s*\(/gmu), assertions: count(content, /\bt\.(?:Error|Errorf|Fatal|Fatalf|Fail|FailNow)\s*\(/gu), skipped: count(content, /\bt\.Skip(?:f|Now)?\s*\(/gu), focused: 0 };
}
function inventoryRust(path, content) {
  return { path, framework: "cargo test", suites: [], tests: matches(content, /#\[test\]\s*(?:async\s+)?fn\s+(\w+)/gu), assertions: count(content, /\bassert(?:_eq|_ne)?!\s*\(/gu), skipped: count(content, /#\[ignore(?:\([^)]*\))?\]/gu), focused: 0 };
}
function matches(content, regex) {
  return [...content.matchAll(regex)].map((match) => match[1] ?? "unnamed");
}
function count(content, regex) {
  return [...content.matchAll(regex)].length;
}
function sum(files, key) {
  return files.reduce((value, file) => value + file[key].length, 0);
}
async function walk(directory) {
  const result = [];
  for (const entry of await (0, import_promises6.readdir)(directory, { withFileTypes: true })) {
    if (ignored.has(entry.name)) continue;
    const path = (0, import_node_path6.join)(directory, entry.name);
...
// src/requirements-core.ts
var import_promises7 = require("node:fs/promises");
var import_node_path7 = require("node:path");
var import_yaml3 = __toESM(require_dist(), 1);
async function loadRequirements(repository, base) {
  let text;
  try {
  ...
  if (!text) {
    try {
      text = await (0, import_promises7.readFile)((0, import_node_path7.join)(repository, ".n0", "requirements.yml"), "utf8");
    } catch {
      return [];
    }
  }
  const document = (0, import_yaml3.parse)(text);
  return (document?.requirements ?? []).map((item, index) => ({
    ...
    severity: oneOf2(item.severity, ["critical", "high", "medium", "low"], "medium"),
    mandatory: item.mandatory !== false,
    evidence: stringArray2(item.evidence),
    ...Array.isArray(item.paths) ? { paths: stringArray2(item.paths) } : {},
    ...Array.isArray(item.tests) ? { tests: stringArray2(item.tests) } : {},
...
// src/requirements.ts
function evaluateRequirements2(options) {
  const evaluation = evaluateRequirements(options);
...
function renderHtmlReport(result) {
  ...
  const findings = result.findings.length ? result.findings.map((finding2) => findingHtml(finding2)).join("") : "<p class=empty>No policy findings.</p>";
  const requirements = result.requirements?.length ? `<table><thead><tr><th>Requirement</th><th>Status</th><th>Reason</th></tr></thead><tbody>${result.requirements.map((item) => `<tr><td><strong>${escapeHtml(item.id)}</strong><br>${escapeHtml(item.statement)}</td><td>${escapeHtml(item.status)}</td><td>${escapeHtml(item.reason)}</td></tr>`).join("")}</tbody></table>` : "<p class=empty>No requirements were configured.</p>";
  const matrix = result.matrix?.cells ? `<div class=matrix>${["C0T0", "C1T0", "C0T1", "C1T1"].map((key) => `<div><span>${key}</span><strong>${result.matrix?.cells[key]?.status ?? "NOT_RUN"}</strong></div>`).join("")}</div><p>${escapeHtml(result.matrix.interpretation)}</p>` : "<p class=empty>Matrix execution was not requested.</p>";
...
function renderSarif(result) {
  const rules = [...new Map(result.findings.map((finding2) => [finding2.ruleId, {
    id: finding2.ruleId,
    shortDescription: { text: finding2.kind.replaceAll("_", " ") }
  }])).values()];
  return {
    version: "2.1.0",
    ...
    runs: [{
      ...
      results: result.findings.map((finding2) => ({
        ruleId: finding2.ruleId,
        level: finding2.severity === "blocking" || finding2.severity === "critical" ? "error" : finding2.severity === "warning" ? "warning" : "note",
        message: { text: finding2.message },
        ...finding2.path ? { locations: [{ physicalLocation: { artifactLocation: { uri: finding2.path }, region: { startLine: finding2.line ?? 1 } } }] } : {}
      }))
    }]
  };
}
function findingHtml(finding2) {
  return `<article class="finding ${escapeHtml(finding2.severity)}"><strong>${escapeHtml(finding2.ruleId)} \u2014 ${escapeHtml(finding2.kind.replaceAll("_", " "))}</strong>${finding2.path ? `<p>${escapeHtml(finding2.path)}${finding2.line ? `:${finding2.line}` : ""}</p>` : ""}<p>${escapeHtml(finding2.message)}</p>${finding2.evidence ? `<code>${escapeHtml(finding2.evidence)}</code>` : ""}</article>`;
}
...
async function signHash(hash2, privateKeyPath) {
  const privateKey = await (0, import_promises8.readFile)(privateKeyPath, "utf8");
  const publicKey = (0, import_node_crypto3.createPublicKey)(privateKey).export({ type: "spki", format: "pem" }).toString();
  return { algorithm: "Ed25519", publicKey, signature: (0, import_node_crypto3.sign)(null, Buffer.from(hash2, "utf8"), privateKey).toString("base64"), signedHash: hash2 };
}
...
// src/proof-core.ts
async function produceProofPack(options) {
  const runId = `n0_${options.result.generatedAt.replace(/[-:.TZ]/gu, "").slice(0, 14)}_${options.result.head.slice(0, 8)}`;
  const directory = (0, import_node_path8.join)(options.outputRoot ?? (0, import_node_path8.join)(options.result.repository, ".n0", "runs"), runId);
  await (0, import_promises9.mkdir)((0, import_node_path8.join)(directory, "logs"), { recursive: true });
...
`;
    await (0, import_promises9.writeFile)((0, import_node_path8.join)(directory, name), text, "utf8");
    hashes[name] = sha256(text);
  }
  const html = renderHtmlReport(options.result);
  await (0, import_promises9.writeFile)((0, import_node_path8.join)(directory, "report.html"), html, "utf8");
...
# stderr
${command.stderr}`;
      await (0, import_promises9.writeFile)((0, import_node_path8.join)(directory, "logs", `${filename}.log`), log, "utf8");
  ...
  const manifest = {
    schemaVersion: "1.0",
    runId,
    baseCommit: options.result.base,
    headCommit: options.result.head,
    startedAt: options.result.generatedAt,
    isolationMode: options.result.isolationMode ?? "audit",
    policyHash: sha256(canonicalJson(options.config)),
    proofHash,
    files: hashes
  };
  await (0, import_promises9.writeFile)((0, import_node_path8.join)(directory, "manifest.json"), `${canonicalJson(manifest)}
`, "utf8");
  if (options.signingKey) await (0, import_promises9.writeFile)((0, import_node_path8.join)(directory, "signature.json"), `${canonicalJson(await signHash(proofHash, options.signingKey))}
`, "utf8");
...
function sortValue(value) {
  ...
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).filter(([, item]) => item !== void 0).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, sortValue(item)]));
  return value;
}
function safeEnvironmentSummary() {
  return { platform: process.platform, arch: process.arch, node: process.version, ci: process.env.CI === "true" };
}

// src/proof.ts
async function produceProofPack2(options) {
  const pack = await produceProofPack(options);
  await (0, import_promises10.writeFile)((0, import_node_path9.join)(pack.directory, "badge.svg"), renderBadge(options.result.decision), "utf8");
  return pack;
}
...
async function verifyRepository(options) {
  ...
  const loaded = await loadTrustedConfig2(repository, base);
  const config = structuredClone(loaded.config);
  if (options.mode) config.execution.mode = options.mode;
  if (options.timeoutMs) config.execution.timeoutSeconds = Math.max(1, Math.ceil(options.timeoutMs / 1e3));
  ...
  const findings = detectIntegrityFindings2(patch, files);
  applyPolicy(findings, config.policies.blockOn, config.policies.warnOn);
  let adapter;
  let plan;
  let runs = [];
  let matrix = { status: "NOT_RUN", cells: {}, interpretation: "Audit-only mode." };
  let baseInventory;
  let headInventory;
  if (config.execution.mode === "audit") {
    ...
    try {
      ...
      if (adapter) {
        ...
        runs = [
          await executePlan2({ directory: snapshots.base, ref: base, snapshot: "base", plan, config }),
          await executePlan2({ directory: snapshots.head, ref: head, snapshot: "head", plan, config })
        ];
  ...
  const requirementEvaluation = evaluateRequirements2({ definitions: requirementDefinitions, changedFiles: files, runs, matrix });
  for (const requirement of requirementDefinitions) {
    const result2 = requirementEvaluation.results.find((item) => item.id === requirement.id);
    if (requirement.mandatory && result2 && result2.status !== "PASS" && result2.status !== "WAIVED") {
      findings.push({
        ruleId: "N0-REQ-001",
        kind: "mandatory_requirement_failed",
        severity: config.policies.blockOn.includes("mandatory_requirement_failed") ? "blocking" : "warning",
        message: `${requirement.id}: ${result2.reason}`
      });
    }
  }
  const result = {
  ...
  if (options.writeProof) {
    const pack = await produceProofPack2({
      result,
      config,
...
function applyPolicy(findings, blockOn, warnOn) {
  for (const finding2 of findings) {
    if (blockOn.includes(finding2.kind)) finding2.severity = "blocking";
    else if (warnOn.includes(finding2.kind)) finding2.severity = "warning";
  }
}
function addInventoryFindings(findings, base, head) {
  if (head.totals.skipped > base.totals.skipped && !findings.some((finding2) => finding2.kind === "test_skip_added")) {
  ...
  if (head.totals.focused > base.totals.focused && !findings.some((finding2) => finding2.kind === "focused_test_added")) {
...
function counterfactualFromMatrix(matrix) {
...
function appendCounterfactualFinding(findings, result) {
  if (result.status === "SURVIVED" && !findings.some((finding2) => finding2.kind === "non_discriminating_test")) findings.push({ ruleId: "N0-EVIDENCE-001", kind: "non_discriminating_test", severity: "blocking", message: result.summary });
...
function decisionFor(findings) {
  if (findings.some((finding2) => finding2.severity === "blocking" || finding2.severity === "critical")) return "BLOCK";
  if (findings.some((finding2) => finding2.severity === "warning" || finding2.severity === "high" || finding2.severity === "medium")) return "WARN";
  return "PASS";
}
function evidenceFor(findings, counterfactual, runs) {
  if (findings.some((finding2) => finding2.severity === "blocking" && finding2.kind !== "non_discriminating_test") || runs.some((run) => run.snapshot === "head" && run.status === "FAIL")) return "CONTRADICTED";
  if (counterfactual.evidenceStatus === "SUPPORTED") return "SUPPORTED";
  return "INCONCLUSIVE";
}
async function createSnapshotPair(repository, base, head) {
  const root = await (0, import_promises11.mkdtemp)((0, import_node_path10.join)((0, import_node_os4.tmpdir)(), "n0-snapshots-"));
  const baseDirectory = (0, import_node_path10.join)(root, "base");
  const headDirectory = (0, import_node_path10.join)(root, "head");
  await addWorktree3(repository, baseDirectory, base);
  try {
    await addWorktree3(repository, headDirectory, head);
  } catch (error) {
    await removeWorktree3(repository, baseDirectory);
    throw error;
  }
  return {
    base: baseDirectory,
    head: headDirectory,
    cleanup: async () => {
      await removeWorktree3(repository, headDirectory);
      await removeWorktree3(repository, baseDirectory);
      await (0, import_promises11.rm)(root, { recursive: true, force: true });
    }
  };
}
async function addWorktree3(repository, path, ref) {
  const result = await runCommand("git", ["worktree", "add", "--detach", path, ref], { cwd: repository });
  if (result.exitCode !== 0) throw new Error(result.stderr.trim() || "Could not create snapshot worktree.");
}
async function removeWorktree3(repository, path) {
  await runCommand("git", ["worktree", "remove", "--force", path], { cwd: repository }).catch(() => void 0);
}
...
async function verifyRepository2(options) {
  ...
  if (mode === "audit") return await verifyRepository({ ...options, mode: "audit", testCommand: void 0 });
  return await verifyRepository(options);
}
async function configuredMode(options) {
  ...
  return (await loadTrustedConfig2(repository, base)).config.execution.mode;
}
...
async function loadClaims(path) {
  const text = await (0, import_promises12.readFile)(path, "utf8");
  const extension = (0, import_node_path11.extname)(path).toLowerCase();
  if (extension === ".json") return normalizeStructured(JSON.parse(text), path);
  if (extension === ".yml" || extension === ".yaml") return normalizeStructured((0, import_yaml4.parse)(text), path);
  return normalizeMarkdown(text, path);
}
function normalizeMarkdown(text, source = "markdown") {
...
async function loadTrustedWaivers(repository, base) {
  let text;
  try {
  ...
  const document = (0, import_yaml5.parse)(text);
...
function applyWaivers(findings, waivers, now = /* @__PURE__ */ new Date()) {
  for (const finding2 of findings) {
    const waiver = waivers.find((item) => item.rule === finding2.ruleId && /* @__PURE__ */ new Date(`${item.expires}T23:59:59Z`) >= now && (!item.path || finding2.path && globMatch2(finding2.path, item.path)));
    if (!waiver) continue;
    finding2.waived = true;
    finding2.severity = "info";
    finding2.message = `${finding2.message} WAIVED by ${waiver.approvedBy} until ${waiver.expires}: ${waiver.reason}`;
  }
}
function globMatch2(path, pattern) {
  const regex = pattern.replace(/[.+^${}()|[\]\\]/gu, "\\$&").replaceAll("**", "\0").replaceAll("*", "[^/]*").replaceAll("\0", ".*");
...
async function runChangedLineMutations(options) {
  ...
  const root = await (0, import_promises13.mkdtemp)((0, import_node_path12.join)((0, import_node_os5.tmpdir)(), "n0-mutants-"));
  const directory = (0, import_node_path12.join)(root, "head");
  const results = [];
  try {
    ...
    if (options.plan.install) {
      const install = await runShellCommand(options.plan.install, { cwd: directory, timeoutMs: options.config.execution.timeoutSeconds * 1e3, env: sanitizedEnvironment("mutation-install", directory) });
      if (install.exitCode !== 0) return candidates.map((candidate) => ({ ...candidate, status: "ERROR", durationMs: 0 }));
    }
    for (const candidate of candidates) {
      const path = (0, import_node_path12.join)(directory, ...candidate.path.split("/"));
      const original = await (0, import_promises13.readFile)(path, "utf8");
      ...
      await (0, import_promises13.mkdir)((0, import_node_path12.dirname)(path), { recursive: true });
      await (0, import_promises13.writeFile)(path, lines.join("\n"), "utf8");
      const started = Date.now();
      const run = await runShellCommand(options.plan.test, { cwd: directory, timeoutMs: options.config.execution.timeoutSeconds * 1e3, env: sanitizedEnvironment(candidate.id, directory) });
...
${run.stderr}`;
      ...
      await (0, import_promises13.writeFile)(path, original, "utf8");
    }
    return results;
  } catch {
    return candidates.map((candidate) => ({ ...candidate, status: "ERROR", durationMs: 0 }));
  } finally {
    await runCommand("git", ["worktree", "remove", "--force", directory], { cwd: options.repository }).catch(() => void 0);
    await (0, import_promises13.rm)(root, { recursive: true, force: true });
  }
}
function generateCandidates(patch, files) {
  ...
  const operations = [
    [/\btrue\b/u, "true", "false"],
    [/\bfalse\b/u, "false", "true"],
    [/>=/u, ">=", ">"],
    [/<=/u, "<=", "<"],
    [/===/u, "===", "!=="],
    [/==/u, "==", "!="],
    [/\breturn\s+null\b/u, "null", "[]"]
  ];
  const result = [];
...
// src/verify-enhanced.ts
async function verifyRepository3(options) {
  const result = await verifyRepository2({ ...options, writeProof: false });
  const loaded = await loadTrustedConfig2(result.repository, result.base);
  const config = structuredClone(loaded.config);
  if (options.mode) config.execution.mode = options.mode;
  if (options.timeoutMs) config.execution.timeoutSeconds = Math.max(1, Math.ceil(options.timeoutMs / 1e3));
  if (options.testCommand) config.commands.test = options.testCommand;
  const claims = options.claimsPath ? await loadClaims((0, import_node_path13.resolve)(options.claimsPath)) : [];
  ...
  if (result.isolationMode === "worktree" && config.negativeControl.enabled) {
    ...
    if (adapter) {
      ...
      if (survived.length) result.findings.push({
        ruleId: "N0-MUT-001",
  ...
  result.decision = decisionFor2(result.findings);
  result.evidenceStatus = evidenceFor2(result, claimEvidence);
  if (options.writeProof) {
    const pack = await produceProofPack2({
      result,
      config,
...
function evaluateClaimEvidence(claims, result) {
  ...
  if (result.changedFiles.length) types.add("file_change");
  if (result.changedFiles.some((file) => file.category === "test" && file.status !== "deleted")) types.add("test_diff");
  if (result.runs?.some((run) => run.snapshot === "head" && run.status === "PASS")) {
    types.add("runtime_test");
    types.add("clean_test_run");
  }
  if (result.matrix?.cells.C1T0?.status === "PASS") types.add("regression_test");
  if (result.counterfactual.status === "COUNTERFACTUAL_REJECTED" || result.mutations?.some((mutation) => mutation.status === "KILLED")) types.add("negative_test");
...
function decisionFor2(findings) {
  const active = findings.filter((finding2) => !finding2.waived);
  if (active.some((finding2) => finding2.severity === "blocking" || finding2.severity === "critical")) return "BLOCK";
  if (active.some((finding2) => ["warning", "high", "medium"].includes(finding2.severity))) return "WARN";
  return "PASS";
}
function evidenceFor2(result, claims) {
  if (claims.some((claim) => claim.status === "contradicting") || result.findings.some((finding2) => !finding2.waived && finding2.severity === "blocking" && finding2.kind !== "non_discriminating_test")) return "CONTRADICTED";
  ...
  if (claimsComplete && requirementsComplete) return "SUPPORTED";
  return "INCONCLUSIVE";
}

// src/verify.ts
async function verifyRepository4(options) {
  const result = await verifyRepository3({ ...options, claimsPath: options.claimsPath ?? process.env.N0_CLAIMS, writeProof: false });
  if (result.runs?.some((run) => run.status === "BLOCKED" || run.status === "TIMEOUT") && !result.findings.some((finding2) => finding2.kind === "execution_blocked")) {
  ...
  const activeBlocker = result.findings.some((finding2) => !finding2.waived && finding2.severity === "blocking" && finding2.kind !== "non_discriminating_test");
  ...
  if (options.writeProof) {
    ...
    if (options.timeoutMs) config.execution.timeoutSeconds = Math.max(1, Math.ceil(options.timeoutMs / 1e3));
    if (options.testCommand) config.commands.test = options.testCommand;
    const pack = await produceProofPack2({ result, config, ...options.proofOutput ? { outputRoot: options.proofOutput } : {}, ...options.signingKey ? { signingKey: options.signingKey } : {} });
    result.proofDirectory = pack.directory;
  }
  return result;
}
...
async function main() {
  ...
  const event = process.env.GITHUB_EVENT_PATH ? JSON.parse(await (0, import_promises14.readFile)(process.env.GITHUB_EVENT_PATH, "utf8")) : void 0;
  ...
  const requestedMode = process.env.INPUT_MODE || "worktree";
  ...
  try {
    const result = await verifyRepository4({ cwd: workspace, ...base ? { base } : {}, head, mode, writeProof: true });
...
${result.findings.map((finding2) => `- **${finding2.ruleId}** ${finding2.message}`).join("\n")}
`;
    if (process.env.GITHUB_STEP_SUMMARY) await (0, import_promises14.appendFile)(process.env.GITHUB_STEP_SUMMARY, summary, "utf8");
    if (process.env.GITHUB_OUTPUT) await (0, import_promises14.appendFile)(process.env.GITHUB_OUTPUT, `decision=${result.decision}
...
async function updatePullRequestComment(event, body) {
