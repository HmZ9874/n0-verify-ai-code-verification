...
import { verifyRepository as verifyCore, type VerifyOptions as CoreVerifyOptions } from "./verify-core.js";
import type { EvidenceNode, Finding, PolicyDecision, VerificationResult } from "./model.js";
import { loadClaims, type Claim } from "./claims.js";
...
function evaluateClaimEvidence(claims: Claim[], result: VerificationResult): EvidenceNode[] {
  ...
  if (result.runs?.some((run) => run.snapshot === "head" && run.status === "PASS")) {
    types.add("runtime_test"); types.add("clean_test_run");
  }
...
function decisionFor(findings: Finding[]): PolicyDecision {
  ...
  if (active.some((finding) => finding.severity === "blocking" || finding.severity === "critical")) return "BLOCK";
  if (active.some((finding) => ["warning", "high", "medium"].includes(finding.severity))) return "WARN";
  return "PASS";
}

function evidenceFor(result: VerificationResult, claims: EvidenceNode[]): VerificationResult["evidenceStatus"] {
  if (claims.some((claim) => claim.status === "contradicting") || result.findings.some((finding) => !finding.waived && finding.severity === "blocking" && finding.kind !== "non_discriminating_test")) return "CONTRADICTED";
