...
test("verifies a Python unittest change through the four-way matrix", async (context) => {
  ...
  try {
    if ((await runCommand(python, ["--version"], { cwd: process.cwd() })).exitCode !== 0) return context.skip("Python is unavailable");
  } catch { return context.skip("Python is unavailable"); }
