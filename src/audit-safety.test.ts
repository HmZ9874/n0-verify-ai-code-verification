...
test("audit mode never executes an explicitly supplied test command", async () => {
  ...
  try {
    await git(directory, ["init", "--quiet"]); await git(directory, ["config", "user.email", "test@n0.invalid"]); await git(directory, ["config", "user.name", "N0 Test"]);
    await mkdir(join(directory, "src")); await writeFile(join(directory, "src", "a.js"), "export const a = 1;\n");
    await git(directory, ["add", "."]); await git(directory, ["commit", "--quiet", "-m", "base"]);
    await writeFile(join(directory, "src", "a.js"), "export const a = 2;\n"); await git(directory, ["add", "."]); await git(directory, ["commit", "--quiet", "-m", "head"]);
...
async function git(cwd: string, args: string[]): Promise<void> { const result = await runCommand("git", args, { cwd }); if (result.exitCode !== 0) throw new Error(result.stderr); }
