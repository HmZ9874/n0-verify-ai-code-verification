...
function demoFromPatch(name, nameStatus, patch) {
...
function parseNameStatus(value) {
    return value.split(/\r?\n/u).filter(Boolean).map((line) => { const [code = "", path = ""] = line.split("\t"); const status = code === "A" ? "added" : code === "D" ? "deleted" : "modified"; return { path, status, category: classifyFile(path) }; });
}
async function nonDiscriminatingTestDemo() {
    const directory = await mkdtemp(join(tmpdir(), "n0-demo-"));
    try {
        await git(directory, ["init", "--quiet"]);
        await git(directory, ["config", "user.email", "demo@n0.invalid"]);
        await git(directory, ["config", "user.name", "N0 Demo"]);
        ...
        await git(directory, ["add", "."]);
        await git(directory, ["commit", "--quiet", "-m", "candidate"]);
        const base = await resolveRef(directory, "HEAD~1");
        const head = await resolveRef(directory, "HEAD");
        const files = await changedFiles(directory, base, head);
        const result = await runCounterfactual({ repository: directory, base, head, files, testCommand: "node --test", timeoutMs: 30_000 });
        const findings = result.status === "SURVIVED" ? [{ ruleId: "N0-EVIDENCE-001", kind: "non_discriminating_test", severity: "blocking", message: result.summary }] : [];
...
async function git(cwd, args) { const result = await runCommand("git", args, { cwd }); if (result.exitCode !== 0)
    throw new Error(result.stderr || "Git command failed in demo."); }
