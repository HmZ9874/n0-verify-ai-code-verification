...
import { detectAdapter, goAdapter, javascriptAdapter, pythonAdapter, rustAdapter } from "./adapters.js";
...
`],
    ...
    if (options.ci && options.ci !== "github")
        throw new Error("--ci currently supports only github");
    if (options.ci === "github")
        files.push([join("..", ".github", "workflows", "n0-verify.yml"), githubWorkflow()]);
...
async function selectAdapter(cwd, language) {
    if (!language)
        return await detectAdapter(cwd, defaultConfig);
    const aliases = {
    ...
    if (!adapter)
        throw new Error(`Unsupported language hint: ${language}`);
    return adapter;
}
