import { verifyRepository as verifyEnhanced } from "./verify-enhanced.js";
...
export async function verifyRepository(options) {
    ...
    if (!activeBlocker && claimEvidence.length > 0 && claimEvidence.every((evidence) => evidence.status === "supporting") && mandatory.every((requirement) => requirement.status === "PASS" || requirement.status === "WAIVED"))
        result.evidenceStatus = "SUPPORTED";
    if (process.env.N0_STRICT === "true" && result.decision === "WARN")
    ...
    if (options.writeProof) {
        ...
        const pack = await produceProofPack({ result, config, ...(options.proofOutput ? { outputRoot: options.proofOutput } : {}), ...(options.signingKey ? { signingKey: options.signingKey } : {}) });
