...
export async function verifyRepository(options) {
    ...
    if (config.execution.mode === "audit") {
        ...
        if (adapter && plan)
            matrix = await runFourWayMatrix({ repository, base, head, files, plan, config });
    }
...
function applyPolicy(findings, blockOn, warnOn) {
    for (const finding of findings) {
        if (blockOn.includes(finding.kind))
            finding.severity = "blocking";
        else if (warnOn.includes(finding.kind))
...
function counterfactualFromMatrix(matrix) {
    if (matrix.status === "NOT_RUN")
        return { status: "NOT_RUN", evidenceStatus: "INCONCLUSIVE", summary: matrix.interpretation };
    const head = matrix.cells.C1T1;
    const baseWithHeadTests = matrix.cells.C0T1;
    if (head?.status !== "PASS")
        return { status: "HEAD_FAILED", evidenceStatus: "CONTRADICTED", summary: "Candidate code with candidate tests did not pass." };
    if (baseWithHeadTests?.status === "PASS")
        return { status: "SURVIVED", evidenceStatus: "INCONCLUSIVE", summary: "Changed tests pass on both base and head code, so they do not distinguish the change." };
    if (baseWithHeadTests?.status === "FAIL")
        return { status: "COUNTERFACTUAL_REJECTED", evidenceStatus: "INCONCLUSIVE", summary: "Changed tests reject base code. Framework-specific assertion reachability is still required for SUPPORTED status." };
...
function appendCounterfactualFinding(findings, result) {
    if (result.status === "SURVIVED" && !findings.some((finding) => finding.kind === "non_discriminating_test"))
        findings.push({ ruleId: "N0-EVIDENCE-001", kind: "non_discriminating_test", severity: "blocking", message: result.summary });
    if (result.status === "HEAD_FAILED")
        findings.push({ ruleId: "N0-RUN-001", kind: "candidate_tests_failed", severity: "blocking", message: result.summary });
    if (result.status === "INVALID_EXPERIMENT")
        findings.push({ ruleId: "N0-RUN-002", kind: "counterfactual_invalid", severity: "warning", message: result.summary });
}
function decisionFor(findings) {
    if (findings.some((finding) => finding.severity === "blocking" || finding.severity === "critical"))
        return "BLOCK";
    if (findings.some((finding) => finding.severity === "warning" || finding.severity === "high" || finding.severity === "medium"))
        return "WARN";
    return "PASS";
}
function evidenceFor(findings, counterfactual, runs) {
    if (findings.some((finding) => finding.severity === "blocking" && finding.kind !== "non_discriminating_test") || runs.some((run) => run.snapshot === "head" && run.status === "FAIL"))
        return "CONTRADICTED";
    if (counterfactual.evidenceStatus === "SUPPORTED")
        return "SUPPORTED";
    return "INCONCLUSIVE";
}
async function createSnapshotPair(repository, base, head) {
    ...
    try {
        await addWorktree(repository, headDirectory, head);
    }
    catch (error) {
...
async function addWorktree(repository, path, ref) {
    ...
    if (result.exitCode !== 0)
        throw new Error(result.stderr.trim() || "Could not create snapshot worktree.");
}
...
//# sourceMappingURL=verify-core-legacy.js.map