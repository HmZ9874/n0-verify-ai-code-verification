...
try {
    if (command === "init")
        ...
        await explainCommand(rest);
    else if (command === "report")
        await reportCommand(rest);
    else if (command === "baseline")
        await baselineCommand(rest);
...
async function checkCommand(args) {
    ...
    if (values.mode && !["audit", "worktree", "container"].includes(values.mode))
        throw new Error("--mode must be audit, worktree, or container");
    ...
    if (values.strict && result.decision === "WARN")
        result.decision = "BLOCK";
    const json = JSON.stringify(result, null, 2);
    if (values.output)
        await writeFile(resolve(values.output), `${json}\n`, "utf8");
...
async function explainCommand(args) {
    const [ruleId] = args;
    if (!ruleId)
        throw new Error("Usage: n0-verify explain RULE-ID");
...
async function reportCommand(args) {
    ...
    if (!run || basename(run) !== run)
        throw new Error("A valid run id is required.");
    const names = { html: "report.html", json: "proof.json", sarif: "report.sarif.json" };
    const name = names[values.format ?? "html"];
    if (!name)
        throw new Error("--format must be html, json, or sarif");
...
async function baselineCommand(args) {
    ...
    if (subcommand === "create" || subcommand === "update") {
        ...
        console.log(JSON.stringify(await inspectBaseline(repository), null, 2));
    else
        throw new Error("Usage: n0-verify baseline create|inspect|update");
}
async function proofCommand(args) {
    ...
    if (subcommand === "verify") {
        ...
        if (!directory)
            throw new Error("Usage: n0-verify proof verify DIRECTORY");
        ...
        throw new Error("Usage: n0-verify proof verify DIRECTORY | proof keygen");
}
async function benchCommand(args) {
    ...
    if (result.falseNegatives || result.falsePositives)
        process.exitCode = 1;
}
...
function numberOption(value, name) { const parsed = Number(value); if (!Number.isFinite(parsed) || parsed <= 0)
    throw new Error(`${name} must be a positive number`); return parsed; }
function printHelp() {
