...
async function main() {
    ...
    if (process.env.INPUT_STRICT === "true")
        process.env.N0_STRICT = "true";
    try {
...
`;
        ...
        if (process.env.GITHUB_OUTPUT)
            await appendFile(process.env.GITHUB_OUTPUT, `decision=${result.decision}\nevidence_status=${result.evidenceStatus}\nproof_directory=${result.proofDirectory ?? ""}\n`, "utf8");
        if (process.env.INPUT_COMMENT !== "false")
            await updatePullRequestComment(event, summary).catch((error) => console.warn(`Could not update PR comment: ${error instanceof Error ? error.message : String(error)}`));
        console.log(summary);
...
async function updatePullRequestComment(event, body) {
    ...
    if (!commentsResponse.ok)
        throw new Error(`GitHub comments lookup returned ${commentsResponse.status}`);
    ...
    if (!response.ok)
        throw new Error(`GitHub comment update returned ${response.status}`);
}
void main();
