...
function testFile(status = "modified") {
...
-"test": "node --test"
+"test": "node --test || true"`;
    const file = { path: "package.json", status: "modified", category: "dependency" };
...
test("deleted test files are blocking", () => {
    ...
    assert.deepEqual(findings.map(({ kind, severity }) => ({ kind, severity })), [{ kind: "test_file_deleted", severity: "blocking" }]);
});
