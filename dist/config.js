...
function validateConfigShape(value) {
    ...
    if (execution.mode !== undefined && !["audit", "worktree", "container"].includes(String(execution.mode)))
        throw new Error("execution.mode must be audit, worktree, or container.");
    for (const key of ["timeout_seconds", "timeoutSeconds", "max_memory_mb", "maxMemoryMb", "max_cpu", "maxCpu"]) {
        if (execution[key] !== undefined && (typeof execution[key] !== "number" || !Number.isFinite(execution[key]) || Number(execution[key]) < 0))
            throw new Error(`execution.${key} must be a non-negative number.`);
    }
    const commands = isRecord(value.commands) ? value.commands : {};
    for (const [key, command] of Object.entries(commands))
        if (command !== undefined && typeof command !== "string")
            throw new Error(`commands.${key} must be a string.`);
    ...
    for (const key of ["block_on", "blockOn", "warn_on", "warnOn", "critical_paths", "criticalPaths"]) {
        if (policies[key] !== undefined && (!Array.isArray(policies[key]) || !policies[key].every((item) => typeof item === "string")))
            throw new Error(`policies.${key} must be an array of strings.`);
    }
}
