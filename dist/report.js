export function renderHtmlReport(result) {
    ...
    const matrix = result.matrix?.cells
        ? `<div class=matrix>${["C0T0", "C1T0", "C0T1", "C1T1"].map((key) => `<div><span>${key}</span><strong>${result.matrix?.cells[key]?.status ?? "NOT_RUN"}</strong></div>`).join("")}</div><p>${escapeHtml(result.matrix.interpretation)}</p>`
