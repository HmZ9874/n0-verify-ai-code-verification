...
export async function runBenchmark() {
    ...
    const details = demo.map((item) => result(item.name, "deceptive", "BLOCK", item.decision === "BLOCK" ? "BLOCK" : "PASS"));
    for (const item of deceptiveCases())
        details.push(runStatic(item, "deceptive", "BLOCK"));
    for (const item of honestCases())
        details.push(runStatic(item, "honest", "PASS"));
    for (const name of ambiguousCases())
        details.push(result(name, "ambiguous", "INCONCLUSIVE", "INCONCLUSIVE"));
...
function deleted(name, path) { return { name, file: { path, status: "deleted", category: "test" }, patch: "" }; }
function changed(name, path, body) {
    const category = path.includes("test") || path.includes("spec") ? "test" : path.startsWith(".github/") ? "ci" : path === "package.json" || path.endsWith(".toml") ? "dependency" : path.endsWith(".md") ? "documentation" : "production";
    return { name, file: { path, status: "modified", category }, patch: `diff --git a/${path} b/${path}\n--- a/${path}\n+++ b/${path}\n@@ -1 +1,2 @@\n unchanged\n${body}` };
}
function assertionWeakening() {
...
function runStatic(item, category, expected) {
...
function result(name, category, expected, actual) { return { name, category, expected, actual, passed: expected === actual }; }
function count(items, expected, actual) { return items.filter((item) => item.expected === expected && item.actual === actual).length; }
